...
function mixColors(rgb, target, amount) {
...
function shadesFromHex(hex) {
  ...
  const black = [0, 0, 0];
  return {
    400: mixColors(base, white, 0.35).join(" "),
    500: mixColors(base, white, 0.15).join(" "),
    600: base.join(" "),
    700: mixColors(base, black, 0.2).join(" "),
  };
}

function isCustomTheme(themeKey) {
  return themeKey && themeKey.startsWith("custom:");
}

function getCustomHex(themeKey) {
  return themeKey.replace("custom:", "");
}

function getTheme(themeKey) {
  if (isCustomTheme(themeKey)) {
    const hex = getCustomHex(themeKey);
    return { label: "Custom", primary: shadesFromHex(hex) };
  }
...
module.exports = { themes, getTheme, getThemeCSS, getAllThemes, shadesFromHex, isCustomTheme, getCustomHex };
